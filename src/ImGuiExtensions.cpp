...
struct GroupPanelData
{
    ...
    GroupPanelData()
    {
        ...
        NewFrameHook.Callback = [](ImGuiContext*, ImGuiContextHook* hook) { IM_ASSERT(static_cast<GroupPanelData*>(hook->UserData)->TitleItemSet == false); };
...
void EndGroupPanel()
{
    ...
    GetWindowDrawList()->AddRect(itemMin - framePadding, itemMax + framePadding, ImColor(GetStyleColorVec4(ImGuiCol_Border)), framePadding.x * 0.75f);

    Dummy(ImVec2(0.f, framePadding.y + GetFrameHeight() * 0.5f));
}

// ── Multi-keybind recording state ─────────────────────────────────────────────
struct MultiKeybindRecordState
{
    int                   targetCount   = 1;
    int                   recordedCount = 0;
    std::vector<KeyCombo> pending;
};
static std::unordered_map<Keybind*, MultiKeybindRecordState> s_recordStates;

void                                                         KeybindInput(Keybind& keybind, Keybind** keybindBeingModified, const char* tooltip, bool allowMultiple)
{
    ...
    auto&       state         = s_recordStates[&keybind];

    // Sync targetCount from saved combos on first access
    if (state.targetCount == 1 && keybind.keyCombos().size() > 1)
        state.targetCount = (int)keybind.keyCombos().size();

    // ── Bind count combo (1 / 2 / 3) — only for multi-bind keybinds, hidden while recording
    if (allowMultiple && !beingModified)
    {
        SetNextItemWidth(GetFrameHeight() * 1.5f);
        const char* countItems[] = { "1", "2", "3" };
        int         idx          = state.targetCount - 1;
        if (Combo(("##count" + keybind.nickname()).c_str(), &idx, countItems, 3))
            state.targetCount = idx + 1;
        SameLine();
    }
    ...
    // ── Disable styling when another bind is being edited ──────────────────────
    if (disableSet)
    {
    ...
    if (!beingModified && ImGui::Button(("Set" + suffix).c_str(), ImVec2(windowWidth * 0.1f, 0.f)) && !disableSet)
    {
        ...
        *keybindBeingModified = &keybind;
        ...
        auto spBegin                   = std::make_shared<std::function<void()>>();
        *spBegin                       = [&keybind, keybindBeingModified, &state, spBegin]()
        {
            if (!state.pending.empty())
            {
                keybind.UpdateDisplayString(state.pending);
                auto* ds  = keybind.keysDisplayString();
                auto  len = strnlen_s(ds, keybind.keysDisplayStringSize());
                if (len + 6 < keybind.keysDisplayStringSize())
                    strcat_s(ds, keybind.keysDisplayStringSize(), " , ...");
            }

            Input::i().BeginRecordInputs(
                [&keybind, keybindBeingModified, &state, spBegin](KeyCombo kc, bool final)
                {
                    if (!final)
                    {
                        std::vector<KeyCombo> preview = state.pending;
                        preview.push_back(kc);
                        keybind.UpdateDisplayString(preview);
                        return;
                    }

                    state.pending.push_back(kc);
                    ++state.recordedCount;

                    if (state.recordedCount >= state.targetCount)
                    {
                        keybind.keyCombos(state.pending);
    ...
    {
        std::string label = "Recording " + std::to_string(state.recordedCount + 1) + "/" + std::to_string(state.targetCount) + "..." + suffix;
        if (ImGui::Button(label.c_str(), ImVec2(windowWidth * 0.1f, 0.f)))
        {
            Input::i().CancelRecordInputs();
            *keybindBeingModified = nullptr;
            state.pending.clear();
            state.recordedCount = 0;
            keybind.UpdateDisplayString();
        }
    }
...
ImTimelineResult     ImGuiTimelineEvent(const char* str_id, const char* display_name, ImTimelineRange& values, bool selected)
{
    ...
    if (IsItemActivated())
        GetStateStorage()->SetFloat(id, GetIO().MouseClickedPos[0].x - (cursor_pos.x + win->Size.x * f32(values[0] + 0.5f) / f32(s_max_timeline_value)));
...
void ImGuiEndTimeline(i32 line_count, i32* lines, ImVec2* mouseTop, i32* mouseNumber)
{
    ...
    for (i32 i = 0; i < line_count; ++i)
    {
        ...
        a.x += lines ? f32(lines[i] + 1.f) / f32(s_max_timeline_value) * GetAvailableSpace().x : f32(i) * GetAvailableSpace().x / f32(line_count);
        ImVec2 b = a;
        b.y      = start.y;
        win->DrawList->AddLine(a, b, line_color);
        char tmp[256];
...
bool Detail::CloseButton::operator()(const char* id, f32 scale, bool includeScrollbars) const
{
    ...
    ImGui::SameLine();
    ImGui::SetCursorPosX(ImGui::GetWindowWidth() - size() - (includeScrollbars ? (ImGui::GetScrollX() + ImGui::GetStyle().ScrollbarSize) : 0.f));
    return ImGui::Button(std::format("{}##{}", ICON_FA_TIMES, id).c_str());
}

f32 Detail::CloseButton::size() const
{
    GW2_ASSERT(ImGui::GetCurrentContext()->Font == GetBaseCore().fontBold() || ImGui::GetCurrentContext()->Font == GetBaseCore().font());
